...
test("primary uses banana-yellow fill + ink text + 3px stroke border", async ({ mount }) => {
...
test("primary is a full pill at 48px touch height with weight-800 label", async ({ mount }) => {
  const c = await mount(Button, { props: { variant: "primary" }, slots: { default: "Go" } });
...
test("primary has a flat (non-blurred) hard-edged block shadow in stroke black", async ({ mount }) => {
  const c = await mount(Button, { props: { variant: "primary" }, slots: { default: "Go" } });
...
test("accept variant fills mint-green", async ({ mount }) => {
  const c = await mount(Button, { props: { variant: "accept" }, slots: { default: "Yes" } });
  await expect(c).toHaveCSS("background-color", "rgb(16, 185, 129)"); // #10B981
});

test("reject variant fills comic-red", async ({ mount }) => {
  const c = await mount(Button, { props: { variant: "reject" }, slots: { default: "No" } });
  await expect(c).toHaveCSS("background-color", "rgb(239, 68, 68)"); // #EF4444
});

test("secondary variant is white-filled with 3px stroke border (same silhouette)", async ({ mount }) => {
  const c = await mount(Button, { props: { variant: "secondary" }, slots: { default: "More" } });
...
test("disabled button is non-interactive (disabled attribute set)", async ({ mount }) => {
  const c = await mount(Button, { props: { variant: "primary", disabled: true }, slots: { default: "Go" } });
