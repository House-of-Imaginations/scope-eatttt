...
test("pink badge = bubblegum-pink fill, 2px stroke border, radius-sm", async ({
	mount,
}) => {
	const c = await mount(CategoryBadge, {
		props: { tone: "pink" },
		slots: { default: "Animation" },
	});
...
test("label is uppercase 12px / weight 800 / 1px tracking", async ({
	mount,
}) => {
	const c = await mount(CategoryBadge, {
		props: { tone: "pink" },
...
test("badge has no shadow (non-interactive micro-pill, flat against surface)", async ({
	mount,
}) => {
	const c = await mount(CategoryBadge, {
		props: { tone: "pink" },
