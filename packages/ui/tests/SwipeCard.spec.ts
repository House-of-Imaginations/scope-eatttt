...
test("renders rating and price level (star + dollar signs)", async ({
	mount,
}) => {
	const c = await mount(SwipeCard, { props: { restaurant: RESTAURANT } });
...
test("accept button fills mint-green, reject fills comic-red", async ({
	mount,
}) => {
	const c = await mount(SwipeCard, { props: { restaurant: RESTAURANT } });
...
test("ArrowRight emits accept, ArrowLeft emits reject (keyboard a11y)", async ({
	mount,
}) => {
	const swipes: string[] = [];
	const c = await mount(SwipeCard, {
