...
const migration = readFileSync(resolve(import.meta.dirname, "../migrations/0001_outbox_trigger.sql"), "utf8");
const journal = readFileSync(resolve(import.meta.dirname, "../migrations/meta/_journal.json"), "utf8");
...
describe("outbox trigger migration", () => {
  ...
  it("is registered in the drizzle migration journal", () => {
    expect(JSON.parse(journal)).toMatchObject({
      entries: expect.arrayContaining([expect.objectContaining({ idx: 1, tag: "0001_outbox_trigger" })]),
    });
  });
});
