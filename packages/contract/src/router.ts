...
export const contract = {
  session: {
    create: oc.input(CreateSessionInput).output(
      z.object({
    ...
    join: oc
      ...
      .output(z.object({ sessionId: z.string().uuid(), memberId: z.string().uuid() })),
    startSwiping: oc
      .input(MemberScopedSessionInput)
      .output(z.object({ status: z.literal("swiping") })),
    state: oc.input(MemberScopedSessionInput).output(SessionStateSchema.nullable()),
    eventsSince: oc
      .input(SessionIdInput.extend({ afterEventId: z.string().uuid().optional() }))
      .output(z.array(AppEventSchema)),
  },
  swipe: {
    decide: oc.input(SwipeInput).output(
      z.object({
        promoted: z.boolean(),
    ...
    broaden: oc.input(BroadenInput).output(
      z.object({
  ...
  poll: {
    ...
    vote: oc.input(VoteInput).output(
      z.object({
