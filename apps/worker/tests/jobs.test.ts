...
describe("runPollCloseJob", () => {
	it("closes a polling session once and skips repeated runs after it is decided", async () => {
		...
		await expect(
			runPollCloseJob({ repo }, { sessionId: "session-1" }),
		).resolves.toEqual({
			closed: true,
		...
		expect(repo.closed).toEqual([
			{ sessionId: "session-1", winnerCandidateId: "candidate-b" },
		]);
		expect(repo.outbox).toHaveLength(1);
		expect(repo.outbox[0]).toMatchObject({
			type: "poll.closed",
			payload: { winnerCandidateId: "candidate-b" },
		});
	});
});
