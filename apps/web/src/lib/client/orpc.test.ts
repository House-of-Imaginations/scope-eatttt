...
describe("mock api", () => {
  it("creates a session with a join code", async () => {
    ...
    const { sessionId, joinCode } = await api.session.create({
      lat: 1,
      lng: 2,
      cuisines: [],
    });
  ...
  it("promotes a restaurant after 2 distinct-member accepts", async () => {
    ...
    const r1 = await api.swipe.decide({
      sessionId,
      restaurantId: "r1",
      decision: "accept",
    }); // host accept
    const r2 = await api.swipe.decide({
      sessionId,
      restaurantId: "r1",
      decision: "accept",
    }); // 2nd member accept
  ...
  it("tallies votes and closes to a winner", async () => {
    ...
    await api.session.join({ joinCode, displayName: "B" });
    await api.swipe.decide({
      sessionId,
      restaurantId: "r1",
      decision: "accept",
    });
    const p = await api.swipe.decide({
      sessionId,
      restaurantId: "r1",
      decision: "accept",
    });
