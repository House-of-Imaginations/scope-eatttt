...
describe("buildContainer", () => {
  it("uses fake places when PLACES_PROVIDER is fake", () => {
    const container = buildContainer(testEnv({ PLACES_PROVIDER: "fake" }), testOverrides());
  ...
  it("fails fast when Google Places is selected without an API key", () => {
    expect(() => buildContainer(testEnv({ PLACES_PROVIDER: "google" }), testOverrides())).toThrow("GOOGLE_MAPS_API_KEY");
  });
});
