...
import {
	type OutboxNotifyListener,
	type RelayOutboxRow,
	type RelayStore,
	createOutboxNotifyListener,
	startRelay,
} from "../src/lib/server/relay";
...
describe("createOutboxNotifyListener", () => {
	it("uses the direct outbox listener and closes it on stop", async () => {
		...
		let closed = false;
		const listener = createOutboxNotifyListener(
			"postgres://direct",
			async (url, cb) => {
				listened.push(url);
				callback = cb;
...
class MemoryRelayStore implements RelayStore {
	...
	async getPending(id: string): Promise<RelayOutboxRow | null> {
		return (
			this.rows.find((row) => row.id === id && row.dispatchedAt === null) ??
			null
		);
	}
