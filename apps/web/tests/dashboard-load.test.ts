...
class FakeRepo implements Pick<SessionRepo<TransactionContext>, "withTx" | "listSessionsForUser" | "getSessionSummary"> {
...
async function expectRedirect(fn: () => Promise<unknown>, location: string): Promise<void> {
  try {
    await fn();
  } catch (err) {
    expect((err as { status: number; location: string }).status).toBe(302);
    expect((err as { status: number; location: string }).location).toBe(location);
    return;
  }
...
describe("dashboard history load", () => {
  it("redirects logged-out visitors to /login?redirect=/dashboard", async () => {
    ...
    await expectRedirect(() => loadHistory(null, repo), "/login?redirect=/dashboard");
  });
  ...
  it("returns history items for a real user", async () => {
    ...
    await expect(loadHistory({ id: "u1", isAnonymous: false }, repo)).resolves.toEqual({
      items: repo.history,
    });
  });
});
...
describe("dashboard summary load", () => {
  ...
  it("returns { summary: null } for a non-member (no existence leak)", async () => {
    const repo = new FakeRepo();
    repo.summary = null;
    await expect(loadSummary({ id: "u1", isAnonymous: false }, repo, "sess-1")).resolves.toEqual({
      summary: null,
    });
  });
  ...
  it("returns the summary for a member", async () => {
    ...
    await expect(loadSummary({ id: "u1", isAnonymous: false }, repo, repo.summary.id)).resolves.toEqual({
