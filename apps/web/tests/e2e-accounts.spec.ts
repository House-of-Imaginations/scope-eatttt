...
async function newClient(browser: import("@playwright/test").Browser): Promise<BrowserContext> {
  return browser.newContext({ geolocation: SYDNEY, permissions: ["geolocation"] });
}
...
async function createSession(
  page: Page,
  opts: { title: string; timerSec: "60" | "180" | "300" | "600"; threshold: "1" | "2" | "3" | "4" | "5" },
): Promise<string> {
  ...
  await expect(page.getByRole("heading", { name: /lobby/i })).toBeVisible({ timeout: NAV_TIMEOUT });
  return readJoinCode(page);
}

/** Guest join via /join/<code>: type a name, submit, land in the lobby. */
async function joinAsGuest(page: Page, code: string, name: string): Promise<void> {
  await page.goto(`/join/${code}`);
  await expect(page.getByRole("heading", { name: /you're invited to lunch/i })).toBeVisible({
    timeout: NAV_TIMEOUT,
  });
...
test.describe("T9 accounts E2E — signup → create → join → avatars → winner → history", () => {
  test("real account hosts, guests + a signed-in member join, promote → vote → winner → dashboard", async ({
    ...
    await expect(pageA.locator("header").getByText(hostName)).toBeVisible({ timeout: NAV_TIMEOUT });
    ...
    await expect(pageA.getByTestId("history-empty")).toBeVisible({ timeout: NAV_TIMEOUT });

    // ── 2. Client A: create a poll (1-min timer, threshold 1) → /s/{id} ─────
    const code = await createSession(pageA, { title: "Friday Lunch", timerSec: "60", threshold: "1" });
    ...
    await expect(pageA.getByText("Grace Guest")).toBeVisible({ timeout: SSE_TIMEOUT });
    ...
    await expect(pageC.getByRole("heading", { name: /you're invited to lunch/i })).toBeVisible({
      timeout: NAV_TIMEOUT,
    });
    await pageC.getByRole("button", { name: new RegExp(`join as ${memberCName}`, "i") }).click();
    await expect(pageC.getByRole("heading", { name: /lobby/i })).toBeVisible({ timeout: NAV_TIMEOUT });
    await expect(pageA.locator(".members").getByText(memberCName)).toBeVisible({ timeout: SSE_TIMEOUT });
    ...
    for (const name of [hostName, "Grace Guest", memberCName]) {
      await expect(members.getByText(name)).toBeVisible({ timeout: SSE_TIMEOUT });
    }
    // Avatar initials are rendered for the guest (no uploaded image → "GG").
    await expect(members.getByText("GG", { exact: true })).toBeVisible({ timeout: SSE_TIMEOUT });

    // ── 6. Host starts swiping; a single accept (threshold 1) promotes ──────
    await pageA.getByRole("button", { name: /start swiping/i }).click();
    await expect(pageA.getByRole("button", { name: /^accept$/i })).toBeVisible({ timeout: DECK_TIMEOUT });
    await expect(pageB.getByRole("button", { name: /^accept$/i })).toBeVisible({ timeout: DECK_TIMEOUT });

    // One accept hits promoteThreshold=1 → restaurant.promoted fires.
    await pageA.getByRole("button", { name: /^accept$/i }).click();
    await expect(pageA.getByTestId("promote-toast")).toBeVisible({ timeout: SSE_TIMEOUT });

    // Host opens the poll → all contexts transition to the polling screen.
    await pageA.getByRole("button", { name: /open poll/i }).click();
    await expect(pageA.getByRole("heading", { name: /vote/i })).toBeVisible({ timeout: SSE_TIMEOUT });
    await expect(pageB.getByRole("heading", { name: /vote/i })).toBeVisible({ timeout: SSE_TIMEOUT });
    await expect(pageC.getByRole("heading", { name: /vote/i })).toBeVisible({ timeout: SSE_TIMEOUT });
    ...
    await expect(pageA.getByRole("heading", { name: /we have a winner/i })).toBeVisible({
      timeout: SSE_TIMEOUT,
    });
    const winnerName = (await pageA.locator(".winner-name").textContent())?.trim();
    expect(winnerName).toBeTruthy();
    await expect(pageB.locator(".winner-name")).toHaveText(winnerName!, { timeout: SSE_TIMEOUT });
    ...
    await row.click();
    await expect(pageA).toHaveURL(/\/dashboard\/[0-9a-f-]+/, { timeout: NAV_TIMEOUT });
    await expect(pageA.getByTestId("summary")).toBeVisible({ timeout: NAV_TIMEOUT });
    await expect(pageA.getByTestId("summary-winner").locator(".winner-name")).toHaveText(winnerName!);
...
test.describe("T9 accounts E2E — anonymous link preserves member identity", () => {
  test("guest swipes, signs up mid-session, remains the same member with persisted swipes", async ({
    ...
    const code = await createSession(hostPage, { title: "Link Test", timerSec: "60", threshold: "2" });
    ...
    await expect(hostPage.getByText(upgradeName)).toBeVisible({ timeout: SSE_TIMEOUT });
    ...
    await expect(guestPage).toHaveURL(new RegExp(sessionPath.replace(/\//g, "\\/")), {
      timeout: NAV_TIMEOUT,
    });
    await expect(guestPage.getByRole("heading", { name: /lobby/i })).toBeVisible({ timeout: NAV_TIMEOUT });
    ...
    await expect(hostPage.locator(".members > *")).toHaveCount(2, { timeout: SSE_TIMEOUT });
    await expect(hostPage.locator(".members").getByText(upgradeName)).toHaveCount(1);
    ...
    await expect(guestPage.getByTestId("summary")).toBeVisible({ timeout: NAV_TIMEOUT });
