...
import {
	type SessionEventReplayStore,
	createSessionEventStream,
} from "../src/lib/server/sse";
...
describe("createSessionEventStream", () => {
	...
	it("replays multiple pages before live events", async () => {
		...
		const rows = [
			...
			row("00000000-0000-4000-8000-000000000203", {
				type: "poll.closed",
...
class MemoryReplayStore implements SessionEventReplayStore {
	...
	async listSessionEventsAfter(
		...
		return (index === -1 ? sessionRows : sessionRows.slice(index + 1)).slice(
			0,
			this.pageSize,
		);
	}
