...
import { FakePlaces, InMemoryBus, InlineQueue, MemoryCache, type AuthProvider, type AuthUser } from "@scope/core";
...
import { startRelay, type OutboxNotifyListener } from "../src/lib/server/relay";
...
describeE2E("backend happy path", () => {
  it("runs create/join/swipe/poll/vote/winner over oRPC, relay, SSE, and worker job", async () => {
    ...
    try {
      ...
      relayStop = await startRelay({ store: relayStore, bus, listener: new NoopListener(), pollMs: 10 });
      ...
      const stream = await createSessionEventStream({ bus, replayStore: relayStore, sessionId: created.sessionId, heartbeatMs: 0 });
      const reader = stream.getReader();

      await guestClient.session.join({ joinCode: created.joinCode, displayName: "Grace" });
      await hostClient.swipe.decide({ sessionId: created.sessionId, restaurantId: "place-1", decision: "accept" });
      const promoted = await guestClient.swipe.decide({ sessionId: created.sessionId, restaurantId: "place-1", decision: "accept" });
      expect(promoted.promoted).toBe(true);
      const candidateId = promoted.candidate?.id;
      expect(candidateId).toBeDefined();
      expect(await readEvent(reader, "restaurant.promoted")).toMatchObject({ type: "restaurant.promoted", sessionId, candidateId });

      await hostClient.poll.start({ sessionId: created.sessionId, timerMs: 300000 });
      expect(await readEvent(reader, "poll.opened")).toMatchObject({ type: "poll.opened", sessionId });

      await guestClient.poll.vote({ sessionId: created.sessionId, candidateId: candidateId!, value: 1 });
      expect(await readEvent(reader, "vote.cast")).toMatchObject({ type: "vote.cast", sessionId, candidateId });

      await runPollCloseJob({ repo }, { sessionId: created.sessionId });
      expect(await readEvent(reader, "poll.closed")).toMatchObject({ type: "poll.closed", sessionId, winnerCandidateId: candidateId });
...
const guestUser: AuthUser = { id: "guest-user", displayName: "Grace", isAnonymous: false };

function context(user: AuthUser): ORPCContext {
  return { user };
}

function testContainer(repo: DrizzleSessionRepo, relayStore: DrizzleRelayStore, bus: InMemoryBus): AppContainer {
  return {
    config: testEnv(),
    repo,
    bus,
...
async function readEvent(reader: ReadableStreamDefaultReader<Uint8Array>, type: AppEvent["type"]): Promise<AppEvent> {
  const deadline = Date.now() + 3000;
  while (Date.now() < deadline) {
    const chunk = await Promise.race([reader.read(), wait(300).then(() => undefined)]);
    if (!chunk?.value) {
      continue;
    }
...
async function seedUsers(db: ReturnType<typeof createDatabaseClients>["db"]): Promise<void> {
...
async function seedRestaurants(db: ReturnType<typeof createDatabaseClients>["db"]): Promise<void> {
