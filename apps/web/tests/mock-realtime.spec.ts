...
test.describe("mock realtime workflow", () => {
	test("shares mock state across tabs, survives reload, and completes the poll", async ({
		browser,
	}) => {
		...
		await expect(host.getByTestId("invite-link")).toHaveValue(
			new RegExp(`/join/${code}$`),
		);
		...
		await simon.reload();
		await expect(simon.getByRole("heading", { name: /lobby/i })).toBeVisible({
			timeout: NAV_TIMEOUT,
		});
		...
		await expect(host.getByRole("button", { name: /^accept$/i })).toBeVisible({
			timeout: NAV_TIMEOUT,
		});
		await expect(simon.getByRole("button", { name: /^accept$/i })).toBeVisible({
			timeout: NAV_TIMEOUT,
		});

		const firstCard = (
			await host.getByTestId("swipe-card-name").textContent()
		)?.trim();
		...
		await expect(simon.getByTestId("promote-toast")).toBeVisible({
			timeout: NAV_TIMEOUT,
		});

		await host.getByRole("button", { name: /open poll/i }).click();
		await expect(host.getByRole("heading", { name: /vote/i })).toBeVisible({
			timeout: NAV_TIMEOUT,
		});
		await expect(host.getByText(firstCard!)).toBeVisible({
			timeout: NAV_TIMEOUT,
		});
		await expect(simon.getByText(firstCard!)).toBeVisible({
			timeout: NAV_TIMEOUT,
		});
		...
		await expect(
			host.getByRole("heading", { name: /we have a winner/i }),
		).toBeVisible({ timeout: NAV_TIMEOUT });
		await expect(
			simon.getByRole("heading", { name: /we have a winner/i }),
		).toBeVisible({ timeout: NAV_TIMEOUT });
